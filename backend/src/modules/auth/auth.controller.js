...
exports.register = async (req, res) => {
    ...
    try {
        ...
        // Company users start as 'pending'; others as 'active' (but unverified email)
        const status = role === 'company' ? 'pending' : 'active';

        const [result] = await db.query(
            `INSERT INTO users (name, email, password, role_id, status, email_verify_token, email_verify_token_expires)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
            [name, email, hashedPassword, roleRow[0].id, status, verifyToken, tokenExpiry]
        );

        const userId = result.insertId;

        // Save company approval record
        if (role === 'company') {
            if (!company_name) {
        ...
        ${role === 'company' ? '<p><strong>Note:</strong> Your company account requires admin approval after verification.</p>' : ''}
      `,
        });
        ...
        res.status(201).json({
            message:
                role === 'company'
                    ? 'Registration successful. Please verify your email. Your account will be active after admin approval.'
...
exports.verifyEmail = async (req, res) => {
    ...
    try {
       ...
       WHERE email_verify_token = ? AND is_email_verified = 0`,
        ...
        if (new Date() > new Date(user.email_verify_token_expires)) {
...
exports.login = async (req, res) => {
    ...
    try {
       ...
       WHERE u.email = ?`,
        ...
        // Block company users until admin approves
        if (user.role === 'company' && user.status === 'pending') {
            return res.status(403).json({
                message: 'Your company account is pending admin approval. You will be notified once approved.',
            });
        }
